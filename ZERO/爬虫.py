from http import cookiejar
from urllib import request, error
from urllib.parse import urlparse
rooturl="www.baidu.com"

header=""
body=""
cookies=""
parameterRequest="" #请求参数
request=()
retry_count=""#服务器错误 重新发起请求
proxy="" #代理



class TTT(object):
    def get_prepare(self,header,body ,cookies ):# 通用请求头,体,cookies
        
        return 
    def download(self, url, retry_count=3, headers=None, proxy=None, data=None):# retry_count 重试次数
        if url is None:
            return None
        try:
            req = request.Request(url, headers=headers, data=data)
            cookie = cookiejar.CookieJar()
            cookie_process = request.HTTPCookieProcessor(cookie)
            opener = request.build_opener()
            if proxy:
                proxies = {urlparse(url).scheme: proxy}
                opener.add_handler(request.ProxyHandler(proxies))
            content = opener.open(req).read()
        except error.URLError as e:
            print('HtmlDownLoader download error:', e.reason)
            content = None
            if retry_count > 0:
                if hasattr(e, 'code') and 500 <= e.code < 600:
                    #说明是 HTTPError 错误且 HTTP CODE 为 5XX 范围说明是服务器错误，可以尝试再次下载
                    return self.download(url, retry_count-1, headers, proxy, data)
        return content
    
    
