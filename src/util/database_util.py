...
from conf.config import session, DB
...
class Test(unittest.TestCase):
    def test_get_tables(self):
        print(get_tables(session, DB))

    def test_get_cols(self):
        print(get_cols(session, DB, 'dividend_bond'))

    def test_get_pks(self):
        print(get_unique_cols(session, DB, 'dividend_bond'))

    def test_version(self):
        sql = "select version()"
        print(exec_to_list_dict(session, sql))
