    ...
    for table_name in tables:
        ...
        for col_name in res:
            ...
            if "_code" in col_name.lower():
                ...
                sql3 = "UPDATE IGNORE {} SET {}= REPLACE(REPLACE({},'SZA','SZ'),'SHA','SH')".format(table_name, col_name,
                                                                                                    col_name)
                table_data["update_sql"].append(col_name)
                ret.append(sql3)
                del_ret.append("delete from {} where {} like '%SZA' or {} like '%SHA' ".format(table_name, col_name, col_name))
    # dict 转 json 输出文件
