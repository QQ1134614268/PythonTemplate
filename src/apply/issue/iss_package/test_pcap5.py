...
class MyTestPck(Packet):
    ...
    fields_desc = [
        ...
        FieldLenField("field_len_2", None, length_of="field_list_2", fmt="B", count_of=None,
                      adjust=lambda pkt, x: x),  # "fmt: B: 1字节,Byte; H:2字节, I: 4字节"
        FieldListField("field_list_2", [], InnerField("InnerField", ""), length_from=lambda pck: pck.len_2),

        PacketLenField("packet_len", InnerPkt(), InnerPkt, length_from=lambda pck: pck.len_2),

        PacketField("packet_field", None, InnerPkt),
        PacketListField("packet_list", None, InnerPkt, length_from=lambda pck: pck.len_2),
