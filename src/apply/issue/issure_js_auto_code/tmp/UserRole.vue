  ...
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="用户名" prop="deptId">
        <el-input v-model="queryForm.role" placeholder="请输入用户名"/>
      </el-form-item>

      <el-form-item label="用户名" prop="deptId">
        <el-input v-model="queryForm.userId" placeholder="请输入用户名"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="用户名" prop="role" width="55" align="center"></el-table-column>

      <el-table-column label="用户名" prop="userId" width="55" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="用户名" prop="deptId">
          <el-input v-model="form.role" placeholder="请输入用户名"/>
        </el-form-item>
...
import { USER_ROLE_CREATE_API, USER_ROLE_PAGE_API, USER_ROLE_UPDATE_API, USER_ROLE_DELETE_API, USER_ROLE_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        role: [{required: false, trigger: "blur", message: "用户名",}],
        userId: [{required: false, trigger: "blur", message: "用户名",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(USER_ROLE_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(USER_ROLE_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(USER_ROLE_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(USER_ROLE_DELETE_API, row.id);
