...
import { MEMO_CREATE_API, MEMO_PAGE_API, MEMO_UPDATE_API, MEMO_DELETE_API, MEMO_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
    async init() {
      let response = await getJson2(MEMO_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(MEMO_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(MEMO_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(MEMO_DELETE_API, row.id);
