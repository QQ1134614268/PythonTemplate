<template>
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="绩效考核结束时间" prop="deptId">
        <el-input v-model="queryForm.endTime" placeholder="请输入绩效考核结束时间"/>
      </el-form-item>

      <el-form-item label="等级" prop="deptId">
        <el-input v-model="queryForm.level" placeholder="请输入等级"/>
      </el-form-item>

      <el-form-item label="得分" prop="deptId">
        <el-input v-model="queryForm.score" placeholder="请输入得分"/>
      </el-form-item>

      <el-form-item label="绩效考核开始时间" prop="deptId">
        <el-input v-model="queryForm.startTime" placeholder="请输入绩效考核开始时间"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="绩效考核结束时间" prop="endTime" width="55" align="center"></el-table-column>

      <el-table-column label="等级" prop="level" width="55" align="center"></el-table-column>

      <el-table-column label="得分" prop="score" width="55" align="center"></el-table-column>

      <el-table-column label="绩效考核开始时间" prop="startTime" width="55" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="绩效考核结束时间" prop="deptId">
          <el-input v-model="form.endTime" placeholder="请输入绩效考核结束时间"/>
        </el-form-item>
        ...
        <el-form-item label="绩效考核开始时间" prop="deptId">
          <el-input v-model="form.startTime" placeholder="请输入绩效考核开始时间"/>
        </el-form-item>
...
import { PERFORMANCE_CREATE_API, PERFORMANCE_PAGE_API, PERFORMANCE_UPDATE_API, PERFORMANCE_DELETE_API, PERFORMANCE_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        endTime: [{required: false, trigger: "blur", message: "绩效考核结束时间",}],
        level: [{required: false, trigger: "blur", message: "等级",}],
        score: [{required: false, trigger: "blur", message: "得分",}],
        startTime: [{required: false, trigger: "blur", message: "绩效考核开始时间",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(PERFORMANCE_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(PERFORMANCE_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(PERFORMANCE_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(PERFORMANCE_DELETE_API, row.id);
