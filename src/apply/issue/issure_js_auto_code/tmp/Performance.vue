...
import { PERFORMANCE_CRATE_API, PERFORMANCE_PAGE_API, PERFORMANCE_UPDATE_API, PERFORMANCE_DELETE_API, PERFORMANCE_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(PERFORMANCE_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(PERFORMANCE_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(PERFORMANCE_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(PERFORMANCE_DELETE, row.id);
        },
    }
};
</script>