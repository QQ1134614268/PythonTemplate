...
import { ORGANIZATION_CREATE_API, ORGANIZATION_PAGE_API, ORGANIZATION_UPDATE_API, ORGANIZATION_DELETE_API, ORGANIZATION_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(ORGANIZATION_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(ORGANIZATION_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(ORGANIZATION_DELETE_API, row.id);
