  ...
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="" prop="deptId">
        <el-input v-model="queryForm.formName" placeholder="请输入"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="" prop="formName" width="55" align="center"></el-table-column>
...
import { FORM_CREATE_API, FORM_PAGE_API, FORM_UPDATE_API, FORM_DELETE_API, FORM_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        formName: [{required: false, trigger: "blur", message: "",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(FORM_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(FORM_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(FORM_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(FORM_DELETE_API, row.id);
