...
import { FORM_CRATE_API, FORM_PAGE_API, FORM_UPDATE_API, FORM_DELETE_API, FORM_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(FORM_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(FORM_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(FORM_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(FORM_DELETE, row.id);
        },
    }
};
</script>