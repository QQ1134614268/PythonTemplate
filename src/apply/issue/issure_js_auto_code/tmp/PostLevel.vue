<template>
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="类别名称" prop="deptId">
        <el-input v-model="queryForm.levelName" placeholder="请输入类别名称"/>
      </el-form-item>

      <el-form-item label="上级类别Id" prop="deptId">
        <el-input v-model="queryForm.parentPostLevelId" placeholder="请输入上级类别Id"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="类别名称" prop="levelName" width="55" align="center"></el-table-column>

      <el-table-column label="上级类别Id" prop="parentPostLevelId" width="55" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="类别名称" prop="deptId">
          <el-input v-model="form.levelName" placeholder="请输入类别名称"/>
        </el-form-item>

        <el-form-item label="上级类别Id" prop="deptId">
          <el-input v-model="form.parentPostLevelId" placeholder="请输入上级类别Id"/>
        </el-form-item>
...
import { POST_LEVEL_CREATE_API, POST_LEVEL_PAGE_API, POST_LEVEL_UPDATE_API, POST_LEVEL_DELETE_API, POST_LEVEL_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        levelName: [{required: false, trigger: "blur", message: "类别名称",}],
        parentPostLevelId: [{required: false, trigger: "blur", message: "上级类别Id",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(POST_LEVEL_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(POST_LEVEL_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(POST_LEVEL_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(POST_LEVEL_DELETE_API, row.id);
