...
import { ENUM_CREATE_API, ENUM_PAGE_API, ENUM_UPDATE_API, ENUM_DELETE_API, ENUM_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(ENUM_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(ENUM_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(ENUM_DELETE_API, row.id);
