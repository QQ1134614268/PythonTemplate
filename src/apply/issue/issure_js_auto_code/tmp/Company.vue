...
import { COMPANY_CREATE_API, COMPANY_PAGE_API, COMPANY_UPDATE_API, COMPANY_DELETE_API, COMPANY_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
    async init() {
      let response = await getJson2(COMPANY_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(COMPANY_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(COMPANY_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(COMPANY_DELETE_API, row.id);
