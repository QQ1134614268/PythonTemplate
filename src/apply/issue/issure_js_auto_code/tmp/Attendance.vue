...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(ATTENDANCE_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(ATTENDANCE_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(ATTENDANCE_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(ATTENDANCE_DELETE, row.id);
        },
    }
};
</script>