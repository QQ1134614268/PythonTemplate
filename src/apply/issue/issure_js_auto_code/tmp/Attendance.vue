...
import { ATTENDANCE_CREATE_API, ATTENDANCE_PAGE_API, ATTENDANCE_UPDATE_API, ATTENDANCE_DELETE_API, ATTENDANCE_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(ATTENDANCE_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(ATTENDANCE_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(ATTENDANCE_DELETE_API, row.id);
