...
import { CALENDAR_CRATE_API, CALENDAR_PAGE_API, CALENDAR_UPDATE_API, CALENDAR_DELETE_API, CALENDAR_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(CALENDAR_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(CALENDAR_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(CALENDAR_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(CALENDAR_DELETE, row.id);
        },
    }
};
</script>