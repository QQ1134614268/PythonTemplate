<template>
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="组织类型描述" prop="deptId">
        <el-input v-model="queryForm.description" placeholder="请输入组织类型描述"/>
      </el-form-item>

      <el-form-item label="组织状态; 1:开启, 0:不开启" prop="deptId">
        <el-input v-model="queryForm.state" placeholder="请输入组织状态; 1:开启, 0:不开启"/>
      </el-form-item>

      <el-form-item label="组织类型名称" prop="deptId">
        <el-input v-model="queryForm.typeName" placeholder="请输入组织类型名称"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="组织类型描述" prop="description" width="55" align="center"></el-table-column>

      <el-table-column label="组织状态; 1:开启, 0:不开启" prop="state" width="55" align="center"></el-table-column>

      <el-table-column label="组织类型名称" prop="typeName" width="55" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="组织类型描述" prop="deptId">
          <el-input v-model="form.description" placeholder="请输入组织类型描述"/>
        </el-form-item>

        <el-form-item label="组织状态; 1:开启, 0:不开启" prop="deptId">
          <el-input v-model="form.state" placeholder="请输入组织状态; 1:开启, 0:不开启"/>
        </el-form-item>

        <el-form-item label="组织类型名称" prop="deptId">
          <el-input v-model="form.typeName" placeholder="请输入组织类型名称"/>
        </el-form-item>
...
import { ORGANIZATION_TYPE_CREATE_API, ORGANIZATION_TYPE_PAGE_API, ORGANIZATION_TYPE_UPDATE_API, ORGANIZATION_TYPE_DELETE_API, ORGANIZATION_TYPE_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        description: [{required: false, trigger: "blur", message: "组织类型描述",}],
        state: [{required: false, trigger: "blur", message: "组织状态; 1:开启, 0:不开启",}],
        typeName: [{required: false, trigger: "blur", message: "组织类型名称",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(ORGANIZATION_TYPE_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(ORGANIZATION_TYPE_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(ORGANIZATION_TYPE_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(ORGANIZATION_TYPE_DELETE_API, row.id);
