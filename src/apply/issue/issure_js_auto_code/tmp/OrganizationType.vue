...
import { ORGANIZATION_TYPE_CRATE_API, ORGANIZATION_TYPE_PAGE_API, ORGANIZATION_TYPE_UPDATE_API, ORGANIZATION_TYPE_DELETE_API, ORGANIZATION_TYPE_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(ORGANIZATION_TYPE_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(ORGANIZATION_TYPE_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(ORGANIZATION_TYPE_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(ORGANIZATION_TYPE_DELETE, row.id);
        },
    }
};
</script>