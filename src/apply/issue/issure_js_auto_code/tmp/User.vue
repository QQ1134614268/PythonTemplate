...
import { USER_CREATE_API, USER_PAGE_API, USER_UPDATE_API, USER_DELETE_API, USER_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
    async init() {
      let response = await getJson2(USER_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(USER_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(USER_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(USER_DELETE_API, row.id);
