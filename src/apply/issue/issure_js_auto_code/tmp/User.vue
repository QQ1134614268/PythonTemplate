...
import { USER_CRATE_API, USER_PAGE_API, USER_UPDATE_API, USER_DELETE_API, USER_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(USER_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(USER_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(USER_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(USER_DELETE, row.id);
        },
    }
};
</script>