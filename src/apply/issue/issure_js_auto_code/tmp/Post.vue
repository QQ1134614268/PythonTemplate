...
import { POST_CRATE_API, POST_PAGE_API, POST_UPDATE_API, POST_DELETE_API, POST_DELETE_BATCH_API } from "@/api/api";
...
export default {
    ...
    methods: {
        async init() {
            let response = await this.$get2(POST_PAGE, this.queryForm);
            ...
            this.$refs["form"].validate(valid => {
                if (valid) {
                    if (this.form.id == null) {
                        this.postJson2(POST_CRATE, this.form);
                        this.$message.success("修改成功");
                    } else {
                        this.postJson2(POST_UPDATE, this.form);
                        this.$message.success("修改成功");
                    }
        ...
        // 批量删除或者单个删除
        handleDelete(row) {
            this.postJson2(POST_DELETE, row.id);
        },
    }
};
</script>