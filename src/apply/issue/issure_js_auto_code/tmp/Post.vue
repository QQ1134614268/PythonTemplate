<template>
  <div class="app-container">
    <el-form :model="queryForm" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="职位级别" prop="deptId">
        <el-input v-model="queryForm.level" placeholder="请输入职位级别"/>
      </el-form-item>

      <el-form-item label="组织Id" prop="deptId">
        <el-input v-model="queryForm.orgId" placeholder="请输入组织Id"/>
      </el-form-item>

      <el-form-item label="职位名称" prop="deptId">
        <el-input v-model="queryForm.postName" placeholder="请输入职位名称"/>
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData">
      ...
      <el-table-column label="职位级别" prop="level" width="55" align="center"></el-table-column>

      <el-table-column label="组织Id" prop="orgId" width="55" align="center"></el-table-column>

      <el-table-column label="职位名称" prop="postName" width="55" align="center"></el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="职位级别" prop="deptId">
          <el-input v-model="form.level" placeholder="请输入职位级别"/>
        </el-form-item>
...
import { POST_CREATE_API, POST_PAGE_API, POST_UPDATE_API, POST_DELETE_API, POST_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        level: [{required: false, trigger: "blur", message: "职位级别",}],
        orgId: [{required: false, trigger: "blur", message: "组织Id",}],
        postName: [{required: false, trigger: "blur", message: "职位名称",}],
  ...
  methods: {
    async init() {
      let response = await getJson2(POST_PAGE_API, this.queryForm);
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(POST_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(POST_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(POST_DELETE_API, row.id);
