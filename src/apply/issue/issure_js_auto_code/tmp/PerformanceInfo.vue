...
import { PERFORMANCE_INFO_CREATE_API, PERFORMANCE_INFO_PAGE_API, PERFORMANCE_INFO_UPDATE_API, PERFORMANCE_INFO_DELETE_API, PERFORMANCE_INFO_DELETE_BATCH_API } from "@/api/api";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(async valid => {
        if (valid) {
          if (this.form.id == null) {
            await postJson2(PERFORMANCE_INFO_CREATE_API, this.form);
            this.$message.success("修改成功");
          } else {
            await postJson2(PERFORMANCE_INFO_UPDATE_API, this.form);
            this.$message.success("修改成功");
          }
    ...
    async handleDelete(row) {
      await postJson2(PERFORMANCE_INFO_DELETE_API, row.id);
