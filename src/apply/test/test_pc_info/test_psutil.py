...
class TestPsutil(unittest.TestCase):
    ...
    def test1(self):
        name = "longsongpong"
        schedule.every(5).minutes.do(TestPsutil.test1_get_info, name)
    ...
    def test1_get_info(self):
        ...
        for process in psutil.process_iter():
            try:
                ...
                process_list.append(PidInfo(
                    pid=process.pid, cmdline=cmdline, create_time=now,
                ))
        ...
        for net_info in psutil.net_connections():
            l_addr, r_addr = "", ""
            try:
                l_addr = f"{net_info.laddr.ip}:{net_info.laddr.port}"
                r_addr = f"{net_info.raddr.ip}:{net_info.raddr.port}"
