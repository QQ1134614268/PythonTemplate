...
FROM (select t1.fund_id,
      ...
      WHERE t1.biz_date = '2022-03-23'
        ...
        and t1.data_label = 0) t1
         ...
         LEFT JOIN (SELECT fund_id,SUM(scr.clear_amt - scr.creditrepay - scr.creditrepayunfrz) AS sum_fund_id_rz_balance
                    from sec_contract_real scr
                    WHERE scr.updated_at > '2020-12-31'
                      and scr.credit_direct = 0
                      and scr.data_label = 0
                    GROUP BY scr.fund_id
) scr on scr.fund_id = t1.fund_id
